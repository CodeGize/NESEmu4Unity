...
namespace dotNES
{
    partial class PPU
    {
        ...
        public readonly byte[] RawBitmap = new byte[GameWidth * GameHeight * 4];
        ...
        private void ProcessBackgroundForPixel(int cycle, int scanline)
        {
            if (cycle < 8 && !F.DrawLeftBackground || !F.DrawBackground && scanline != -1)
            {
                ...
                // corner case
                var val = _palette[ReadByte(0x3F00 + ((F.BusAddress & 0x3F00) == 0x3F00 ? F.BusAddress & 0x001F : 0)) & 0x3F];
                SetRawBitmap(_bufferPos, val);
                return;
            }
            ...
            if (scanline != -1)
            {
                ...
                SetRawBitmap(_bufferPos, _palette[ReadByte(0x3F00u + paletteEntry) & 0x3F]);
                //RawBitmap[_bufferPos] = _palette[ReadByte(0x3F00u + paletteEntry) & 0x3F];
            }
        }

        private void SetRawBitmap(uint _bufferPos,uint val)
        {
            /*
             * c.r = inv * ();
                c.g = inv * ((val >> 8) & 0xFF);
                c.b = inv * (val & 0xFF);
             */
            RawBitmap[_bufferPos * 4 + 0] = 255;//a
            RawBitmap[_bufferPos * 4 + 1] = (byte)((val >> 16) & 0xFF);//r
            RawBitmap[_bufferPos * 4 + 2] = (byte)((val >> 8) & 0xFF);//g
            RawBitmap[_bufferPos * 4 + 3] = (byte)(val & 0xFF);// b
        }
            ...
            for (int idx = _spriteCount * 4 - 4; idx >= 0; idx -= 4)
            {
                ...
                if (color > 0)
                {
                    ...
                    if (F.DrawBackground && (front || backgroundPixel == 0))
                    {
                        if (scanline != -1)
                        {
                            SetRawBitmap(_bufferPos, _palette[ReadByte(0x3F10 + palette * 4 + color) & 0x3F]);
        ...
        public void ProcessCycle(int scanline, int cycle)
        {
            ...
            if (_cpuSyncCounter + 1 == 3)//PPU每3次循环，CPU运转一次
            {
